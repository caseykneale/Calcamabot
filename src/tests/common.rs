    ...
    match result {
        Value::Scalar(n) => {
            if expected.is_infinite() {
                assert!(
                    n.is_infinite() && (expected.is_sign_positive() == n.is_sign_positive()),
                    "Expected {}, got {}",
                    expected,
                    n
                );
    ...
    match result {
        Value::Matrix(m) => {
            assert_eq!(
                m.len(),
                1,
                "Expected 1-column matrix (vector), got {} columns",
                m.len()
            );
            let v = &m[0];
    ...
    match result {
        Value::ComplexMatrix(m) => {
            assert_eq!(
                m.len(),
                1,
                "Expected 1-column complex matrix (complex vector), got {} columns",
                m.len()
            );
            let v = &m[0];
            ...
            for (i, (a, b)) in v.iter().zip(expected.iter()).enumerate() {
                assert!(
                    approx_eq(a.im, *b, eps),
                    "Element {} imaginary mismatch: expected {}, got {}",
                    i,
                    b,
                    a.im
                );
                assert!(
                    approx_eq(a.re, 0.0, eps),
                    "Element {} real part should be 0, got {}",
                    i,
                    a.re
                );
            }
        ...
        other => panic!(
            "Expected complex vector (1-column complex matrix), got {:?}",
            other
        ),
    }
}
