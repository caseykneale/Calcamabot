...
As somewhat of a continuation of my [study on what is gained/lost while using LLM's to write code](https://github.com/caseykneale/self_study_single_LLM_exchange) this project was geared at using an agentic harness. I chose to revisit a project that took me a few hours to do many years ago which is a CLI calculator called [`Calcamabob`](https://github.com/caseykneale/Calcamabob). 

`Calcamabob`` project wasn't like a serious test of my skills or abilities at the time. I'm not being shy about it, I simply had never written a real parser before and threw one together. It wasn't that hard because there were awesome examples online and I got to learn a lot while doing it. I was always curious to add more to that project but there really was never a point because I have other tools that calculate well. So why not see how it goes doing that project again with an agentic workflow.

## Experiment Design
The purpose of this exercise was to create an advanced CLI calculator to replace some of my usage of other tools using a syntax I like using agentic AI programming tools. The task was time boxed to 5 days of off-and-on agentic programming while I worked on other tasks to see what I could create and maintain with local models. Note: This is not 40 hours of work or even a near equivalent, more like I wrote a plan, made lunch, did something else and checked in 3 hours later. The total time invested ignoring this human write up, was probably 6 hours or something?
...
## What went well
Surprisingly a lot went well. The agent more or less 2-shotted the original calcamabob projects goals. After that it was up to me to come up with features I actually wanted and how I wanted them done. From what I can tell the "final" result does what I asked of it. It didn't come super easy but I also didn't invest much into it.

One thing I really like is when the parser or the math fails it does throw meaningful enough errors that I can catch issues. Matrix dimensions don't match - nice! Or missing a bracket - oops! It doesn't say where the issues happened but that's okay, it was just for fun.
...
Yes small local models can build interesting things. Definitely. This project plays to a coding models strengths when using Rust though. A ton of projects for parsers and the like exist in the rust ecosystem. So this really gave a coding model it's best chance of success. Who knows maybe even the original calcamabob project was in the training data for this model. My name is in several major LLM providers weights... That said, I have used these tools on other domains and they are OK too.
...
I didn't learn much. I wouldnt have learned more had I of not written a pratt parser before either. That part of the code worked well, that's actually a bummer because I hardly saw the code after a quick review. If something went wrong there 1 month from now after adding a new feature I'd have to figure it all out and it would be harder than if I understood it and wrote it.
...
I like the way small model coding is heading to some extent. I like being able to offload boring boiler plate tasks while focussing on the technical challenges of my hobby projects. With small hobby projects its fine to lick the frosting off the cake and leave the cake behind. This project was more or less something I didn't want to work on so it was interesting to do the whole thing this way. No pure frosting diet for me this week. 

I could have done it better, but it would have taken more time than I afforded. Most of the code issues could be solved with a few more cycles with the agent. I guess if the goal was to `get something` that is a win overall? At the same time its not something I feel like celebrating. 
...
I use numerical scores that range from `0` to `10` a bit in this write up. For technical work my view is the following:
 ...
 - `9`: Small gripes
 - `10`: I likely wouldn't change it if I was given free-range to improve it. 
